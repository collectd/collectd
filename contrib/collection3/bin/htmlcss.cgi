...
## Changelog 
# March 10 2009 <gossner@internode.on.net>
# * our TimeSpans (trivial changes)
# added Qhour (900) seconds this is / will be useful 
# added Quarter (90 day) useful for "formal" reporting work.
# left default selection :)
# * rewrote markup for CSS2 divs and classes only: remove table based markup 
# * created new files for use / evaluation names somewhat arbitary
# * bin/htmlcss.cgi share/html.css 
# objectives were: add id attributes for downstream xml processing (though how useful that is ...) 
# clean consistant layout across graph set sizes
# (tables often break on large pages / browsers)
# ready clarification of graph args and host in large sets
# less eye strain better contrast and larger fonts . (though not a big issue)
...
HTML
  ...
  <head>
    ...
    <link rel="stylesheet" href="../share/html.css" type="text/css" media="screen,projection,print" title="Clean CSS" />
    <link rel="alternate stylesheet" href="../share/style.css" type="text/css" title="Graphs Without Borders"/>
<!-- the alt style sheet simply drops all div and class based markup which is exactly the same as no style sheet (for now)-->
  </head>
  <body>
HTML
  $html_started = 1;
...
sub show_selector
{
  ...
  print <<HTML;
 <div class="sectionWrapper" id="top">
    <form action="${\script_name ()}" method="get" id="form0">
      <fieldset>
...
HTML
  ...
  print <<HTML;
	...
	<input type="submit" name="ok_button" value="Display" />
     </fieldset>
   </form>
</div>
HTML
...
#print STDOUT Data::Dumper->Dump ([$types], ['types']);
  ...
  for (sort (keys %$types))
  {
    ...
    for (my $i = 0; $i < $graphs_num; $i++)
    {
      ...
      print "\n  <h1 class=\"typeTitle\">$type</h1>\n" if ($i == 0);

      print qq#    <div id=\"graph$i\" class=\"graphWrapper\">\n    <img src="$url" title="$args"/>\n    <p class="graphTitle">$type</p>\n    <p class="graphArgs" id="args$i">$args</p>\n    </div>\n#;
