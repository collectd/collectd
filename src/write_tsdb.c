  ...
  if (vl->meta) {
    ...
    if (temp) {
      ...
      while (NULL != temp) {
        ...
        if (NULL == ptr1) {
          ERROR("write_tsdb plugin: meta_data tag '%s' is missing a '=' char "
                "(host=%s, plugin=%s, type=%s)",
                temp, vl->host, vl->plugin, vl->type);
          sfree(temp);
          break;
        }
        if (NULL == ptr2)
          break; /* OK, no space. Good. */

        if (ptr2 < ptr1) {
          ERROR("write_tsdb plugin: meta_data tag '%s' contains space in key "
                "definition (host=%s, plugin=%s, type=%s)",
                temp, vl->host, vl->plugin, vl->type);
          sfree(temp);
          break;
        }
