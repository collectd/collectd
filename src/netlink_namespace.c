/**
 * collectd - src/netlink_namespace.c
 ...
 *
 * Authors:
 *   SRE Team at Medallia Inc. <eng-sre at medallia.com>
    ...
    const char *type, const char *type_instance)
{
  ...
  for (ir_ignorelist_t *i = ir_ignorelist_head; i != NULL; i = i->next)
  {
    ...
    DEBUG ("netlink_namespace plugin: check_ignorelist: "
...
static int update_iflist (struct ifinfomsg *msg, const char *dev)
{
  ...
  if ((msg->ifi_index >= 0) && ((size_t) msg->ifi_index >= iflist_len))
  {
    ...
    if (temp == NULL)
    {
      ERROR ("netlink_namespace plugin: update_iflist: realloc failed.");
      return (-1);
    }
    ...
    struct ir_link_stats_storage_s *stats)
{
  ...
  {
    DEBUG ("netlink_namespace plugin: Ignoring %s/interface.", dev);
  }
  ...
  {
    DEBUG ("netlink_namespace plugin: Ignoring %s/if_detail.", dev);
  }
    ...
    void *args __attribute__((unused)))
{
  ...
  {
    ERROR ("netlink_namespace plugin: link_filter_cb: Don't know how to handle type %i.",
        nlh->nlmsg_type);
    return MNL_CB_ERROR;
  }
  ...
  mnl_attr_for_each (attr, nlh, sizeof (*ifm))
  {
    ...
    {
      ERROR ("netlink_namespace plugin: link_filter_cb: IFLA_IFNAME mnl_attr_validate failed.");
      return MNL_CB_ERROR;
    }
  ...
  if (dev == NULL)
  {
    ERROR ("netlink_namespace plugin: link_filter_cb: dev == NULL");
    return MNL_CB_ERROR;
  }
#ifdef HAVE_RTNL_LINK_STATS64
  mnl_attr_for_each (attr, nlh, sizeof (*ifm))
  {
    ...
    {
      ERROR ("netlink_namespace plugin: link_filter_cb: IFLA_STATS64 mnl_attr_validate2 failed.");
      return MNL_CB_ERROR;
    }
  ...
  mnl_attr_for_each (attr, nlh, sizeof (*ifm))
  {
    ...
    {
      ERROR ("netlink_namespace plugin: link_filter_cb: IFLA_STATS mnl_attr_validate2 failed.");
      return MNL_CB_ERROR;
    }
  ...
  DEBUG ("netlink_namespace plugin: link_filter: No statistics for interface %s.", dev);
  return MNL_CB_OK;
...
static int qos_attr_cb (const struct nlattr *attr, void *data)
{
  ...
  if (mnl_attr_get_type (attr) == TCA_STATS_BASIC)
  {
    ...
    {
      ERROR ("netlink_namespace plugin: qos_attr_cb: TCA_STATS_BASIC mnl_attr_validate2 failed.");
      return MNL_CB_ERROR;
    }
...
static int qos_filter_cb (const struct nlmsghdr *nlh, void *args)
{
  ...
  {
    ERROR ("netlink_namespace plugin: qos_filter_cb: Don't know how to handle type %i.",
        nlh->nlmsg_type);
    return MNL_CB_ERROR;
  }
  ...
  {
    DEBUG ("netlink_namespace plugin: qos_filter_cb: Got %s for interface #%i, "
        "but expected #%i.",
  ...
  {
    ERROR ("netlink_namespace plugin: qos_filter_cb: tm->tcm_ifindex = %i "
        ">= iflist_len = %zu",
  ...
  if (dev == NULL)
  {
    ERROR ("netlink_namespace plugin: qos_filter_cb: iflist[%i] == NULL",
        tm->tcm_ifindex);
    return MNL_CB_ERROR;
  }
  ...
  mnl_attr_for_each (attr, nlh, sizeof (*tm))
  {
    ...
    if (mnl_attr_validate (attr, MNL_TYPE_STRING) < 0)
    {
      ERROR ("netlink_namespace plugin: qos_filter_cb: TCA_KIND mnl_attr_validate failed.");
      return MNL_CB_ERROR;
    }
  ...
  if (kind == NULL)
  {
    ERROR ("netlink_namespace plugin: qos_filter_cb: kind == NULL");
    return (-1);
  }
  ...
  DEBUG ("netlink_namespace plugin: qos_filter_cb: got %s for %s (%i).",
  ...
  mnl_attr_for_each (attr, nlh, sizeof (*tm))
  {
    ...
    {
      ERROR ("netlink_namespace plugin: qos_filter_cb: TCA_STATS2 mnl_attr_validate failed.");
      return MNL_CB_ERROR;
    }
  ...
  mnl_attr_for_each (attr, nlh, sizeof (*tm))
  {
    ...
    {
      ERROR ("netlink_namespace plugin: qos_filter_cb: TCA_STATS mnl_attr_validate2 failed.");
      return MNL_CB_ERROR;
    }
  ...
  DEBUG ("netlink_namespace plugin: qos_filter_cb: Have neither TCA_STATS2 nor "
      "TCA_STATS.");
#endif
...
static int ir_config (const char *key, const char *value)
{
  ...
  int fields_num;
  int status = 1;
  char nspath[1024] = "/var/run/netns/";
  ...
  else if (strcasecmp(key, "NSName") == 0) {
    INFO ("netlink_namespace plugin: recevied NSName value %s", fields[0]);
    if ((fields_num < 1) || (fields_num > 2)) {
      ERROR ("netlink_namespace plugin: Invalid number of fields for option "
          "`%s'. Got %i, expected 1 or 2.", key, fields_num);
      return (-1);
...
static int ir_read (void)
{
  ...
  for (size_t ifindex = 1; ifindex < iflist_len; ifindex++)
  {
    ...
    for (size_t type_index = 0; type_index < STATIC_ARRAY_SIZE (type_id); type_index++)
    {
      ...
      {
        DEBUG ("netlink_namespace plugin: ir_read: check_ignorelist (%s, %s, (nil)) "
            "== TRUE", iflist[ifindex], type_name[type_index]);
        continue;
      }

      DEBUG ("netlink_namespace plugin: ir_read: querying %s from %s (%zu).",
...
{
  plugin_register_config ("netlink_namespace", ir_config, config_keys, config_keys_num);
  plugin_register_init ("netlink_namespace", ir_init);
  plugin_register_read ("netlink_namespace", ir_read);
  plugin_register_shutdown ("netlink_namespace", ir_shutdown);
