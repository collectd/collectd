 ...
 * Copyright (C) 2019 Manuel Sanmartin
 *
...
static notify_snmp_target_t *notify_snmp_malloc_target(void) {
  ...
  if (target == NULL) {
    ERROR("notify_snmp plugin: notify_snmp_malloc_target: calloc failed.");
    return NULL;
  }
...
static notify_snmp_oids_t *notify_snmp_malloc_oids(void) {
  ...
  if (oids == NULL) {
    ERROR("notify_snmp plugin: notify_snmp_malloc_oids: calloc failed.");
    return NULL;
  }
  return oids;
}
...
static int notify_snmp_config_set_string(char **ret_string,
                                         oconfig_item_t *ci) {
...
static int notify_snmp_config_set_integer(int *ret_number, oconfig_item_t *ci) {
  if ((ci->values_num != 1) || (ci->values[0].type != OCONFIG_TYPE_NUMBER)) {
    WARNING("notify_snmp plugin: The `%s' config option "
            "needs exactly one number argument.",
            ci->key);
    return -1;
  }
...
static int notify_snmp_oids_append_oid(notify_snmp_oids_t **oids,
  ...
  notify_snmp_oid_t *oids_list =
      realloc((*oids)->list, sizeof(notify_snmp_oid_t) * ((*oids)->len + 1));
  if (oids == NULL) {
...
static notify_snmp_oids_t *notify_snmp_get_default_oids(void) {
  ...
  for (int i = 0; notify_snmp_default_oids[i].string != NULL; i++) {
    int status =
        notify_snmp_oids_append_oid(&oids, notify_snmp_default_oids[i].id,
  ...
  for (int i = 0; i < ci->children_num; i++) {
    ...
    for (int n = 0; notify_snmp_oids_map[n].name != NULL; n++) {
      if (strcasecmp(notify_snmp_oids_map[n].name, child->key) == 0) {
        status = notify_snmp_config_oids_append_oid(
            &oids, notify_snmp_oids_map[n].id, child);
        found = 1;
        break;
      }
    }
    ...
    if (found == 0) {
      WARNING("notify_snmp plugin: Option `%s' not allowed here.", child->key);
      status = -1;
    }
...
static void notify_snmp_exit_session(notify_snmp_target_t *target) {
  ...
  if (target->sess_handle != NULL && !target->sess_reuse) {
    ...
    if (status == 0) {
      ...
      WARNING("notify_snmp plugin: snmp_sess_close error: '%s'.", errstr);
    }
  ...
  if (session->version == SNMP_VERSION_1) {
    ...
    if (oid_enterprise == NULL) {
      ERROR("notify_snmp plugin: notify_snmp_create_pdu "
            "cannot find Enterprise OID for Target %s.",
            target->name);
      return NULL;
    }
    ...
    if (pdu->enterprise == NULL) {
      ERROR("notify_snmp plugin: notify_snmp_create_pdu: malloc failed.");
      snmp_free_pdu(pdu);
      return NULL;
    }
    ...
    notify_snmp_oid_t *oid_trap =
        notify_snmp_oids_get_oid(target->oids, NOTIFY_SNMP_TRAP_OID);

    if (oid_trap == NULL) {
      ERROR("notify_snmp plugin: notify_snmp_create_pdu "
            "cannot find Trap OID for Target %s.",
            target->name);
      return NULL;
    }
    ...
    if (status) {
      ERROR("notify_snmp plugin: Target %s: "
            "snmp_add_var oid sysuptime failed",
            target->name);
      snmp_free_pdu(pdu);
      return NULL;
    }
...
static notification_meta_t *
notify_snmp_notification_meta_get(notification_meta_t *meta, char *name) {
  while (meta != NULL) {
...
static int notify_snmp_sendsnmp(notify_snmp_target_t *target,
                                notification_t *n) /* {{{ */
{
  int status;
  ...
  for (int i = 0; i < oids_len; i++) {
    ...
    case NOTIFY_SNMP_VALUE_OID: {
      ...
      if (meta != NULL && meta->type == NM_TYPE_DOUBLE) {
        status =
            ssnprintf(buffer, sizeof(buffer), "%f", meta->nm_value.nm_double);
        if ((status < 0) || ((size_t)status >= sizeof(buffer))) {
          WARNING("notify_snmp plugin: notify_snmp_sendsnmp:"
...
static int notify_snmp_notification(const notification_t *n,
  ...
  while (target != NULL) {
    ...
    int status = notify_snmp_sendsnmp(target, (notification_t *)n);
