...
There is also and option to disable logging in case your configuration causes a
lot of logging output. If the option is added in the I<GenericJMX plugin> block
it will be active, it needs an argument to parse but it is not used in any way.
For example “yes” can be used as the argument. Note: if you want logging do not
include this option.

=over 4

=item B<DisableLogging> I<yes>
