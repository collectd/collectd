 ...
 *
 * Documentation on used sysfs interface:
...
static const char *config_keys[] = {"ReportByCpu"};
...
static derive_t *cpu_states_times =
    NULL; // array of times spent among all CPUs in particular state
...
static _Bool getvalue(const char *fname, derive_t *value, char *buffer,
                      size_t buffer_size) {
  FILE *fh;
  if ((fh = fopen(fname, "r")) == NULL)
    return (0);
...
static int cpuidle_init(void) {
  ...
  for (num_cpu = 0; !done; ++num_cpu) {
    status = ssnprintf(filename, sizeof(filename),
                       "/sys/devices/system/cpu/cpu%zu/cpuidle", num_cpu);
  ...
  for (i = 0; i < num_cpu; ++i) {
    ...
    while (1) {
      status = ssnprintf(filename, sizeof(filename),
                         "/sys/devices/system/cpu/cpu%zu/cpuidle/state%zu", i,
                         nstate);
  ...
  if (!report_by_cpu) {
    ...
    for (i = 0; i < num_cpu; ++i)
      for (j = 0; j < cpu_states_num[i]; ++j) {
        ...
        status = ssnprintf(
            ...
            "/sys/devices/system/cpu/cpu%zu/cpuidle/state%zu/name", i, j);
        ...
        if (cpu_states_names == NULL ||
            lfind(state_name, cpu_states_names, &num_all_states, sizeof(char *),
                  cmpstring_dir) == NULL) {
          // insert new element
          cpu_states_names =
              realloc(cpu_states_names, sizeof(char *) * (num_all_states + 1));
          if (cpu_states_names == NULL) {
          ...
          if ((cpu_states_names[num_all_states] =
                   strndup(state_name, sizeof(state_name))) == NULL) {
    ...
    if ((cpu_states_times = malloc(num_all_states * sizeof(derive_t))) ==
        NULL) {
...
static void cpuidle_submit_value(const char *plugin_instance,
                                 const char *state_name, derive_t value) {
  value_t values[1];
...
static int cpuidle_read(void) {
  ...
  for (i = 0; i < num_cpu; ++i)
    for (j = 0; j < cpu_states_num[i]; ++j) {
      ...
      if (report_by_cpu) {
        status = ssnprintf(statind, sizeof(statind), "%zu", i);
        ...
        char **ind;
        ind = (char **)bsearch(state_name, cpu_states_names, num_all_states,
                               sizeof(char *), cmpstring_dir);
        if (ind == NULL) {
          ERROR("cpuidle plugin: state %s not found in internal database",
                state_name);
          return (-3);
        }
  ...
  for (i = 0; i < num_all_states; ++i)
    cpuidle_submit_value(NULL, cpu_states_names[i],
                         cpu_states_times[i] / num_cpu);

  if (reported_last_run == 0) {
    WARNING("cpuidle plugin: nothing was reported, possibly cpuidle is not "
            "supported");
    return (-1);
  }
...
void module_register(void) {
  plugin_register_config("cpuidle", cpuidle_config, config_keys,
                         config_keys_num);
