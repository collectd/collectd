...
=item B<AlignRead> B<false>|B<true>

When set to B<true>, calls to read functions of the plugin are time aligned to a
multiple of B<Interval>. This overrides the global B<AlignRead> setting. 

=item B<AlignReadOffset> I<Seconds>

Specifies an offset, in seconds, the call to time-aligned read functions is 
delayed. This setting automatically enables B<AlignRead> for the plugin. By 
...
If enabled, the call to read functions is time aligned to a multiple of the read
interval, which allows round timestamps or the same timestamps across systems 
to be recorded. This option is disabled by default.
