...
Enables the aligned execution of read callbacks, e.g. to obtain round timestamps
or the same timestamps on different systems. The AlignRead float value is in 
seconds. Values smaller than 60 (value mod 60) determine the second of a minute. 
Decimal places determine the millisecond of a second. Values greater or equal 
than 60 additionally determine the minute of an hour (the integer part of value 
divided by 60). To read every full minute set AlignRead to 0 and Interval to 60.
