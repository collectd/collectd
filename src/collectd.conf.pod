...
This plugin has one option B<Path>. It reads file defined by B<Path> option
(for all CPU installed) to get the current CPU frequency. If this file 
does not exist make sure B<cpufreqd> (L<http://cpufreqd.sourceforge.net/>) or 
a similar tool is installed and an "cpu governor" (that's a kernel module) 
is loaded.
...
An optional parameter "Path" is introduced to specify
...
=item B<Path> I<Path>
If option is ommitted the default value 
"/sys/devices/system/cpu/cpu%d/cpufreq/scaling_cur_freq" is used.
