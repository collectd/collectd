...
length with the and B<WriteQueueLimitHigh> option.
...
If you want to deterministically drop the oldest elements instead of random
ones, set B<WriteQueueLimitHigh> and B<WriteQueueLimitLow> to the same value.
...
=head2 Plugin C<write_parquet>

The I<write_parquet plugin> will save values as I<Apache Parquet> file format.
For each metric creates directory with template C<"I<host name>/I<metric family name>/I<metric labels>".>


B<Synopsis:>

 <Plugin write_parquet>
    BaseDir "/home/example/dir"
    FileDuration 7200
    Compression "Off"
    BufferSize 10000
    BufferDuration 600
...
=item B<BaseDir> I<Path>

Path to directory in filesystem to save all files in. Defaults to C<".">.

=item B<FileDuration> I<Seconds>

Minimal amount of time between creating new file to write in. Defaults to C<3600>.

=item B<Compression> I<off|uncompressed|gzip|zstd|brotli>

I<Off> option is alias to I<Uncompressed>
Set the file compression algorithm. Default value is I<Uncompressed>

=item B<CompressionLevel> I<Level>

Ignored when B<Compression> set to I<Uncompressed>.
Set the file compression level. Default value is I<1>

=item B<BufferSize> I<NumRows>

If set to 0, directly writes to file.
Defines maximal number of metric values to be contained before writing.
Each generated file has its own buffer, but together they cannot
contain more than I<BufferSize>

=item B<BufferDuration> I<Seconds>

Determines time for buffered values to be stored.
If the file`s lifetime has expired earlier than buffer`s one,
I<BufferDuration> is ignored and all data will be flushed before closing file.
...
canonical identifier which is used as I<plugin instance>.

=item B<Host> I<Hostname>

The name or IP-address of the host running the I<Redis> instance.
