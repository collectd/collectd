...
You can also use B<Node> to group servers. This is useful for routing
metrics to different servers.
...
The B<Node> block identifies a new servers group. The name for node is
a canonical identifier which is used as I<plugin instance>. It is limited to
64E<nbsp>characters in length. You can put B<Server> entries here.
