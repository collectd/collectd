...
L<http://www.postgresql.org/docs/manuals/>.
  ...
  <Plugin postgresql>
    ...
    <Writer sqlstore>
      ...
      StoreRates true
      # Meta data can be added to the 10th argument as a JSON string
...
A JSON string which stores meta data. This argument is optional, will be added
only when MetaData config is specified.
...
=item B<MetaData> B<false>|B<true>|B<keys>

If set to B<true>, meta data formatted by JSON will be passed to $10 of
Statement. Or you can choose specific meta data by listing each keys instead of
B<true>.
