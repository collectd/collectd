...
own (or the provided I<types.mbus.db>).
...
Are we using serial gateway (B<true>) or network (B<false>).
...
TCP-Port to connect to (when using network gateway).
...
address (integer 1 to 250) or secondary address (16 digits). Multiple slave blocks can be used.
