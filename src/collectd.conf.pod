...
=item B<NotifyInterfaceAdd> I<true|false>

When a new interface is added, its status typically changes from unset to down.
This leads to an event called a first notification. The severity of this first
notification is a OKAY, subsequent notifications are warnings if the state
toggles to down. If NotifyInterfaceAdd is set to false, this first notification
