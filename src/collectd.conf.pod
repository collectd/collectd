...
=head2 Plugin C<nginx_multihost>
...
Synopsis:
  ...
  <Plugin nginx_multihost>
    <Host productionServer>
      URL "http://productionServerUrl/nginx_status"
      User "stats"
      Password "password"
    </Host>

    <Host deployServer>
      URL "http://deployServerUrl/nginx_status"
    </Host>
  </Plugin>

A B<Host> block defines one connection to a nginx server. It accepts a
single argument which specifies the name of the host. None of the other
options are required.

The following options are accepted by the C<nginx_multihost plugin>
inside B<Host> block:
