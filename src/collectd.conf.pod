...
from Freescale (sensor attached to any I2C bus available in the computer, for HW
details 
see I<http://www.freescale.com/webapp/sps/site/prod_summary.jsp?code=MPL115A>).

It was developed and tested under Linux only. The only platform dependency is
the standard Linux i2c-dev interface (the and the particular bus driver has to
...
To filter out sensor noise a simple averaging using floating window of configurable
size is used.
...
=item B<Averaging> I<window_size>

Size of the averaging window. The plugin will use average of the last C<window_size>
measurements (value of 1 means no averaging). Minimal size is 1, maximal 1024.
