...
Read the metric name from the numbered column (identified by the column numbers starting at zero).
...
=item B<LabelFrom> I<key> I<column>

Specifies the column number whose value will be used to create a label.
Can appear multiple times in the B<Result> block.

=item B<ValueFrom> I<column>

Specifies the column (identified by the column numbers starting at zero)
whose content is used as the actual data for the metric are dispatched
to the daemon.  The plugin uses L<strtoll(3)> and L<strtod(3)> to parse
counter and gauge values respectively, so anything supported by those 
functions is supported by the plugin as well. This option is mandatory.
