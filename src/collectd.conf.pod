...
It is possible to omit metrics for a specific NFS version by setting one or
...
You can also limit metrics to specific NFS procedures (e.g., I<write> and
I<commit>) to limit the amount of data generated by specifying a space- or
comma-seperated list to one of the following options.
By default, all procedures are reported. Specifying B<ReportProcs> will limit
reporting to the given procedures for all NFS versions; specifying
B<ReportProcsV>I<n> will limit to the given list only for NFS version I<n>.
E.g. B<ReportProcsV3 "write,commit"> will only report NFS3 procedures
I<write> and I<commit> while still reporting all NFS2 and NFS4 procedures.

=over 4

=item B<ReportProcs> "I<procedure, procedure, ...>"

=item B<ReportProcsV2> "I<procedure, procedure, ...>"

=item B<ReportProcsV3> "I<procedure, procedure, ...>"

=item B<ReportProcsV4> "I<procedure, procedure, ...>"
