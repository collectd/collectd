...
of I<Graphite>, via I<TCP>(optionally using SSL/TLS) or I<UDP> and sends data
via the "line based" protocol (per default using portE<nbsp>2003). The data
...
Protocol to use when connecting to I<Graphite>. Defaults to C<tcp>. Use C<tls>
as protocol name to enable TLS.
...
=item B<CAFile> I<String>

Path to PEM file with CA certificates. If B<Protocol> is C<tls>, this option
must be present.

=item B<CertFile> I<String>

Path to PEM formatted file with certificate for client authentification. In
case B<Protocol> is C<tls>, this option must be present.

=item B<KeyFile> I<String>

Path to PEM formatted file with private key for client authentification. Key
must match certificate in B<CertFile>. In case B<Protocol> is C<tls>, this
