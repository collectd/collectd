...
=head2 Plugin C<systemd>

The I<systemd> plugin collects systemd metrics of specified services and
slices. Metrics are collected if the corresponding accounting flag is true
(see L<systemd.service(5)>). For example, CPUUsageNSec is collected when
CPUAccounting is true. Collected metrics and corresponding flags are
CPUAccounting, CPUUsageNSec, MemoryAccounting, MemoryAvailable, MemoryCurrent,
MemoryPeak, MemorySwapCurrent, MemoryZSwapCurrent, MemorySwapPeak,
IOAccounting, IOReadBytes, IOReadOperations, IOWriteBytes, IOWriteOperations,
IPAccounting, IPEgressBytes, IPEgressPackets, IPIngressBytes, IPIngressPackets,
TasksAccounting, TasksCurrent (see L<org.freedesktop.systemd1(5)>).
...
Sets the service to collect metrics from it.

=item B<Slice> I<slice>

Sets the slice to collect metrics from it.
