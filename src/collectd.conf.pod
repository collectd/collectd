...
=head2 Plugin C<prometheus>

This plugin collects metrics generated by a prometheus webserver.
To see full description of prometheus format, please visit
L<https://prometheus.io/docs/instrumenting/exposition_formats/>

Collectd inner state doesn't support histogram and summary metric types. Thus,
these types stored as gauge metrics.

This plugin has the following config options: 

=over 4

=item B<URL> I<Prometheus_instance_URL>

Sets the URL of the prometheus instance output.
...
=item B<JWTToken> I<JWTToken>

Optional jwt token needed for authentication.
