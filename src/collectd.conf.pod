...
The C<nginx plugin> accepts two styles of configuration:

1. Single nginx instance monitoring configuration:
...
2. A newer form that supports multiple nginx instances:

  <Plugin nginx>
      <Instance production>
          URL "http://production.server/nginx_status"
          User "stats"
          Password "password"
      </Instance>
      <Instance staging>
          URL "http://staging.server/nginx_status"
      </Instance>
  </Plugin>
...
Optional hostname associated with an nginx instance, defaults to the global hostname
