...
the same timestamps on different systems. Together with the 'Interval' option 
B<AlignRead> determines in which minute of an hour, second of a minute and 
fraction of a second read callbacks are triggered. The B<AlignRead> value is in 
seconds. The value modulo 60 determines the second of a minute. The integer 
part of the value divided by 60 determines the minute of an hour. Optionally, 
decimal places can be used to set the fraction of a second. For example, to 
trigger read callbacks every full minute set AlignRead to 0 and Interval to 60. 
Values below zero disable the read alignment (default: -1.0). 
