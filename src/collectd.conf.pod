...
If the value of the metadata attribute is the empty string, then the tag is
not submitted to InfluxDB. In this case, if a tag with the same name is also
generated via the E<lt>B<Format>E<gt> configuration, then that tag is deleted.
