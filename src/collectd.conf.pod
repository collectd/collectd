...
=item B<SampleBasedRates> B<false>|B<true>

Do not calculate per-second rates.  Instead, calculate the difference since
the last sample was taken.  This is useful in cases where a downstream
aggregator will compute time-based rates from the raw data.
