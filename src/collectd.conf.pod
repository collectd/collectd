...
after which a host is considered missing is controlled with the B<HostTimeout>
option.
...
An additional two seconds are added on top of B<HostTimeout> and B<StartupDelay>
automatically to ensure there are no race conditions where a host might be
declared missing but then immediately found.
...
which is twice the value of Interval.
