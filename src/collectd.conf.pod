...
=head2 Plugin C<notify_dbi>

The I<notify_dbi> plugin use the I<dbi> library (L<http://libdbi.sourceforge.net/>) 
to send notifications to various databases, executing I<SQL> statements when 
...
=item B<Query> [B<Failure>|B<Warning>|B<Ok> ...] I<Statement>

Execute the statement when a notification match any of the serverities defined.
