...
The host plugin keeps track of hosts that have been recently seen, and sends
notifications when a host is seen for the first time, and subsequently when
a host has gone missing after not being seen for a period of time.

=over 4

=item B<HostPath> I<path/to/database>

Path and filename of the LMDB hosts database. Defaults to B<hosts>.
