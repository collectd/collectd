...
The C<wmi> plugin reads Windows system data through Windows Management Instrumentation.
...
The configuration consists of one or more B<Query> blocks.

Each B<Query> block contains the following:

=item B<Statement> I<Statement>

The WMI query to execute. It must return columns with name corresponding to what is set
in B<ValuesFrom> and B<InstancesFrom> (if set).
...
A string to prepend to the plugin instance of each dispatched data point.

Optional

Type: string

=item B<InstancesFrom> I<Field>

A field from the result of the WMI query to use to set the plugin instance of the data point.
...
=head3 B<Metric> blocks

=item B<Type> I<Type>

A type from the TypesDB file.

Mandatory

Type: string

=item B<ValuesFrom> I<Field>

A field from the result of the WMI query to use to set the values of the data points.
...
Default: The value retrieved from the field specified in "ValuesFrom".
