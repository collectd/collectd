...
=item B<LISTSTATE> [B<state=>I<E<lt>state<gt>>]

Returns a list of the values available in the value cache together with the
time of the last update, so that querying applications can issue a B<GETVAL>
command for the values that have changed. Each return value consists of the
update time as an epoch value, the identifier and the internal state of the
value, all separated by a space. The update time is the time of the last value,
as provided by the collecting instance and may be very different from the
time the server considers to be "now".
The list can be also filtered by state using the option provided by the command.
Valid values are: OKAY, ERROR,<...>

Example:
  -> | LISTSTATE
  <- | 19 Values found
  <- | 1182204284 myhost/cpu-0/cpu-idle OKAY
  <- | 1182204284 myhost/cpu-0/cpu-nice OKAY
  <- | 1182204284 myhost/cpu-0/cpu-system OKAY
  <- | 1182204284 myhost/cpu-0/cpu-user ERROR
  <- | 1182204284 myhost/interface-eno1/if_dropped OKAY
  <- | 1182204284 myhost/interface-eno1/if_errors OKAY
  <- | 1182204284 myhost/interface-eno1/if_octets OKAY
  <- | 1182204284 myhost/interface-eno1/if_packets ERROR
  <- | 1182204284 myhost/interface-lo/if_dropped OKAY
  <- | 1182204284 myhost/interface-lo/if_errors OKAY
  <- | 1182204284 myhost/interface-lo/if_octets OKAY
  ...
