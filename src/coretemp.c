...
coretemp_findcores ()
{
  ...
  for (i = 0; i < core_count; i++)
  {
    DEBUG ("coretemp: socket=%d core=%d tjmax=%d label=%s", c[i]->socket,
	   c[i]->core, c[i]->tjmax, c[i]->label);
  }
#endif /* COLLECT_DEBUG */
  ...
  for (idx = 0; idx < core_count; idx++)
  {
    ...
    DEBUG
      ("coretemp: MaxValues=%d ValuesDegrees=%d ValuesPercentage=%d idx=%d core=%d socket=%d hwmon=%d tjmax=%d temp=%d pct=%d label=%s",
       ...
       c[idx]->socket, c[idx]->hwmon, c[idx]->tjmax, temp / 1000, pct,
       c[idx]->label);
...
static int
coretemp_config (const char *key, const char *value)
{
