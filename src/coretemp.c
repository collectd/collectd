...
int i;
  for(i=0; i<core_count; i++ ) {
    if ( c[i]->input ) close(c[i]->input);
    if ( c[i]->label ) free(c[i]->label);
    free(c[i]);
  }
  if ( c ) free(c);
  c=0;
...
FILE *f=0;
  ...
  if ( chdir(CORETEMP_PATH) ) {
    ERROR ("coretemp plugin: unable to find coretemp path (%s)", CORETEMP_PATH);
    return(-1);
  }
  
  if ( !(dp = opendir(".")) ) {
    ERROR ("coretemp plugin: unable to diropen() coretemp path (%s)", CORETEMP_PATH);
    return (-1);
  }
  ...
  while ( (de = readdir(dp)) ) {
    ...
    if ( !strncmp(de->d_name, CORETEMP_NAME, CORETEMP_STRLEN) ) {
            ...
            if ( chdir(de->d_name) ) {
              ERROR ("coretemp plugin: unable to chdir into [ %s ] for coretemp data", de->d_name);
              goto error_exit;
            }
            dp2 = opendir(".");
            int hwmon_mode = 1;
            while ( (de2 = readdir(dp2)) ) {
              ...
              if ( !(s = strchr(&de2->d_name[4], '_')) ) continue; 
              *s = 0;
            ...
            while ( dp3 && (de3 = readdir(dp3)) ) {
              ...
              if ( strncmp(de3->d_name, "hwmon", 5) ) continue;
              hwmon = atoi(&de3->d_name[5]);
              if ( chdir(de3->d_name) ) {
                ERROR ("coretemp plugin: unable to chdir into [ %s ] for coretemp data", de3->d_name);
                goto error_exit;
              }
              dp4 = opendir(".");
              ...
              while ( (de4 = readdir(dp4)) ) {
                if ( de4->d_name[0] == '.' ) continue;
                if ( (l=strlen(de4->d_name)) < 5 ) continue;
                if ( !(s = strchr(&de4->d_name[4], '_')) ) continue;
                ...
                *s=0;
                ...
                if ( idx < 0 ) {
                  ...
                  if ( NULL == (c[core_count] = malloc(sizeof(struct coretemp_core))) ) {
                ...
                *b=t;
                if 	( !strcmp(s, "max") ) {
                  ...
                  asprintf(&c[idx]->label, "%s", core_name);
                  s = c[idx]->label;
                  while ( *s ) {
                    if ( *s == ' ' ) *s = '_';
                    ++s;
  ...
  closedir(dp); dp = 0;
    ...
    for(i=0;i<core_count;i++){
      DEBUG("coretemp: socket=%d core=%d tjmax=%d label=%s", c[i]->socket, c[i]->core, c[i]->tjmax, c[i]->label);
    }
  #endif /* COLLECT_DEBUG */
  ...
  error_exit:
    ...
    if ( f ) 	{ fclose(f); f=0; }
    if ( dp )	{ closedir(dp); dp=0; }
    if ( dp2 )	{ closedir(dp2); dp2=0; }
    if ( dp3 )	{ closedir(dp3); dp3=0; }
    if ( dp4 )	{ closedir(dp4); dp4=0; }
...
static void coretemp_submit(const char *temp_type, const char *str_core_id, const char *str_value)
{
  value_t values[1];
...
char temp_str[20];
  ...
  for(idx=0; idx<core_count; idx++) {
    ...
    #if COLLECT_DEBUG
      DEBUG("coretemp: MaxValues=%d ValuesDegrees=%d ValuesPercentage=%d idx=%d core=%d socket=%d hwmon=%d tjmax=%d temp=%d pct=%d label=%s", MaxValues, ValuesDegrees, ValuesPercentage, idx, c[idx]->core, c[idx]->socket, c[idx]->hwmon, c[idx]->tjmax, temp/1000, pct, c[idx]->label);
      //fprintf(fdopen(0, "w"), "coretemp: idx=%d core=%d socket=%d tjmax=%d temp=%d pct=%d label=%s\n", idx, c[idx]->core, c[idx]->socket, c[idx]->tjmax, temp/1000, pct, c[idx]->label);
    #endif /* COLLECT_DEBUG */
    
    if ( 2 == MaxValues ) continue;
...
static int coretemp_config (const char *key, const char *value)
{
  if ( !strcasecmp(key, "MaxValues") ) {
    if ( !strcasecmp(value, "true") ) MaxValues = 1;
    else if ( !strcasecmp(value, "yes") ) MaxValues = 1;
    
    if ( !strcasecmp(value, "no") ) MaxValues = 0;
    else if ( !strcasecmp(value, "false") ) MaxValues = 0;
    else if ( !strcasecmp(value, "none") ) MaxValues = 0;
    
    if ( !strcasecmp(value, "only") ) MaxValues = 2;
  }
  
  if ( !strcasecmp(key, "ValuesPercentage") ) ValuesPercentage = IS_TRUE(value);
  if ( !strcasecmp(key, "ValuesDegrees") ) ValuesDegrees = IS_TRUE(value);  
...
void module_register (void)
{
  ...
  plugin_register_config ("coretemp", coretemp_config, config_keys, config_keys_num);
  ...
  else coretemp_findcores(); //otherwise why waste cycles?
