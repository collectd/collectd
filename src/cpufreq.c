 ...
 *  Distribution utilization for each CPU frequency in milliseconds that this
 *frequency was used per second of wall time
 *
 **/
...
static const char *config_keys[] = {"ReportDistribution", "ReportByCpu"};
...
static int cpufreq_fill_frequencies() {
  ...
  for (size_t i = 0; i < num_cpu; ++i) {
    ...
    status = ssnprintf(
        ...
        "/sys/devices/system/cpu/cpu%zu/cpufreq/stats/time_in_state", i);
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL) {
      ...
      if (numfields != 2) // supported format contains 2 fields
      {
        ...
        ERROR("cpufreq plugin: wrong number of items on a line in %s",
              filename);
        fclose(fp);
      ...
      if (!found) {
        ...
        temp = (long int *)realloc(hertz_all_cpus,
                                   (hertz_size + 1) * sizeof(long int));
        if (temp == NULL) {
          sfree(hertz_all_cpus);
          ERROR("cpufreq plugin: realloc failed for hertz_all_cpus size %zu",
                hertz_size + 1);
          fclose(fp);
          plugin_unregister_read("cpufreq");
          return (0);
...
static int cpufreq_init(void) {
  ...
  while (1) {
    status = ssnprintf(filename, sizeof(filename),
                       "/sys/devices/system/cpu/cpu%zu", num_cpu);
  ...
  INFO("cpufreq plugin: Found %zu CPU%s", num_cpu, (num_cpu == 1) ? "" : "s");
  ...
  if (num_cpu > 0 && report_distribution && !report_by_cpu) {
    ...
    if (time_all_cpus == NULL) {
      ERROR("cpufreq plugin: malloc failed for time_all_cpus size %zu",
            hertz_size);
      plugin_unregister_read("cpufreq");
      return (0);
    }
...
static void cpufreq_submit_distribution_value(const char *plugin_instance,
                                              long int hertz, derive_t value) {
  value_t values[1];
  value_list_t vl = VALUE_LIST_INIT;
...
static int cpufreq_read_distribution() {
  ...
  for (size_t i = 0; i < num_cpu; ++i) {
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL) {
      ...
      if (hertz <= 0) {
        WARNING("cpufreq: something is wrong in %s: hertz = %ld; line in file "
                "is %s %s",
...
static int cpufreq_read(void) {
  ...
  if (reported_last_run == 0) {
    WARNING("cpufreq plugin: nothing was reported, possibly cpufreq or "
            "cpufreq-stat is not supported");
    return (-1);
  }
