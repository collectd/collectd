...
static int uptime_init (void) /* {{{ */
{
	...
	if (ret != 1)
	{
		ERROR ("No value read from "STAT_FILE"");
		return (-1);
	}
	...
	if (boottime == 0)
	{
		ERROR ("btime read from "STAT_FILE", but `boottime' is zero!");
		return (-1);
	}
/* #endif KERNEL_LINUX */
	...
	if (ksp == NULL)
	{
		ERROR ("Cannot find unix:0:system_misc kstat.");
		return (-1);
	}

	if (kstat_read (kc, ksp, NULL) < 0)
	{
		ERROR ("kstat_read failed.");
		return (-1);
	}
	...
	if (knp == NULL)
	{
		ERROR ("kstat_data_lookup (boot_time) failed.");
		return (-1);
	}
	...
	if (boottime == 0)
	{
		ERROR ("kstat_data_lookup returned success, but `boottime' is zero!");
		return (-1);
	}
/* #endif HAVE_LIBKSTAT */
...
# elif HAVE_SYS_SYSCTL_H
	...
	{
		char errbuf[1024];
		ERROR ("No value read from sysctl interface: %s",
			sstrerror (errno, errbuf, sizeof (errbuf)));
		return (-1);
	}
	...
	if (boottime == 0)
	{
		ERROR ("sysctl(3) returned success, but `boottime' is zero!");
		return (-1);
	}
/* #endif HAVE_SYS_SYSCTL_H */
