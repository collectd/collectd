...
Returns a list of all values (by their identifier) available to the
C<unixsock> plugin with their state. It's possibile filter all the values by
state. Each value is printed on its own line. I.E<nbsp>e., this command
returns a list of valid identifiers that may be used with the other commands.
The valid values for the state are: OKAY, ERROR,<...>
